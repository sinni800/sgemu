...
func Marshal(w io.Writer, v interface{}) (err error) {
	...
	err = p.marshalValue(reflect.ValueOf(v), "???", 0)
	p.Flush()
	return err
}

func (p *printer) marshalValue(val reflect.Value, name string, inner int) error {
	if !val.IsValid() {
		return nil
	}
	...
	if (kind == reflect.Slice || kind == reflect.Array) && typ.Elem().Kind() != reflect.Uint8 {
		for i, n := 0, val.Len(); i < n; i++ {
			if err := p.marshalValue(val.Index(i), name, inner); err != nil {
				return err
			}
		}
		return nil
	...
	case reflect.Float32:
		p.WriteString(strconv.FormatFloat(val.Float(), 'g', -1, 32))
	case reflect.Float64:
